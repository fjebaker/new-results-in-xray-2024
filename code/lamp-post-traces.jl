...
function trace_even(m, d, model; δs = deg2rad.(range(0.01, 179.99 * 2, 78)))
...
function plot_paths_xz!(ax, sols; kwargs...)
    ...
    for sol in sols
        plot_path_xz!(ax, sol; color = popfirst!(palette), kwargs...)
    end
end
...
begin
    ...
    Makie.save("presentation/figs/raw/lamp-post.traces.svg", fig)
    fig
end
